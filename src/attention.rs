...
use crate::utils::linear_algebra::{matmul_3d_2d, matmul_4d, apply_attention_mask};
...
impl MultiHeadAttention {
    pub fn new(
        ...
        Self {
            query_weight_t: query_weight.t().to_owned(),
            query_bias,
            key_weight_t: key_weight.t().to_owned(),
            key_bias,
            value_weight_t: value_weight.t().to_owned(),
            value_bias,
            output_weight_t: output_weight.t().to_owned(),
            output_bias,
            num_heads,
            head_dim,
            scale_factor,
    ...
    pub fn forward(&self, hidden: &Array3<f32>, attention_mask: &Array2<f32>) -> Result<Array3<f32>> {
